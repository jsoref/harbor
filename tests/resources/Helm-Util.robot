...
*** Keywords ***
Prepare Helm Cert
    Wait Until Command Success  cp harbor_ca.crt /ca/server.crt
...
Helm Repo Add
    ...
    Wait Until Command Success  helm repo add --ca-file /ca/server.crt --username=${user} --password=${pwd} ${helm_repo_name} ${harbor_url}/chartrepo/${project_name}
